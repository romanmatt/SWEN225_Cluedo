...
public class CluedoGame {
    ...
    public boolean accuse(Accusation accusation) {
        if (accusation.accusedRoom == solutionRoom && accusation.accusedWeapon ==
                solutionWeapon && accusation.accusedCharacter == solutionCharacter) {
            return true;
        } else {
            return false;
        }
                ...
                if (input.equals("Y")) {
                    ...
                    while (!validInput) {
                        System.out.println(player.toString() +
                                " enter a coordinate you can reach in " +
                                steps + " steps separated by a space eg:5 3");
    ...
    public void setupPlayers() {
        for (Player p : activePlayers) {
            ...
            if (p.getName() == CharacterType.COLONELMUSTARD) {
                ((HallwayCell) board.board[1][17]).placePlayer(p);
            }
